...
def filter_pandas_to_insert(
        df: pd.DataFrame,
        last_updated: datetime.date = datetime.date(1900, 1, 1)):
    """Returns filtered pandas df with rows for dates
    after the provided last_updated date."""
    try:
...
def load_pandas_to_bitdotio_flawed(
        df: pd.DataFrame,
        connstr: str,
        scraped_table: str, repo: str):
